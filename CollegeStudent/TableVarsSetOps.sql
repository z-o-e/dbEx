...
/*** My version here ***/
select distinct CS.sID from 
(select sID from Apply where Apply.major = 'CS') CS
where CS.sID not in
(select EECS.sID from
(select distinct A1.sID as sID
from Apply A1, Apply A2
where A1.sID = A2.sID and A1.major='CS' and A2.major = 'EE') EECS);
